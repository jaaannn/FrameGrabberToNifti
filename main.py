...
def main_function():
    ...
    def save_data_to_nifti(*args):
        ...
        os.mkdir(save_path)
        numpy_frontal = np.array(images_frontal)
        numpy_lateral = np.array(images_frontal)
        np.save(save_path + "recording_frontal.npy", numpy_frontal)
        np.save(save_path + "recording_lateral.npy", numpy_lateral)
        nib.save(nib.Nifti1Image(numpy_frontal, np.eye(4)), save_path + "recording_frontal.nii.gz")
        nib.save(nib.Nifti1Image(numpy_lateral, np.eye(4)), save_path + "recording_lateral.nii.gz")
        sys.exit()
